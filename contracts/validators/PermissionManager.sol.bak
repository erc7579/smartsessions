...
contract PermissionManager is ERC7579ValidatorBase, ERC7579ExecutorBase, IPermissionManager {
        ...
        returns (ValidationData vd)
    {
        ...
        if (_isEnableMode(userOp.signature)) {
            ...
            signerId = SignerId.wrap((bytes32(userOp.signature[1:33])).mixinNonce(getNonce(msg.sender)));
        ...
        returns (bytes4 sigValidationResult)
    {
        ...
        SignerId signerId = SignerId.wrap((bytes32(signature[1:33])).mixinNonce(getNonce(msg.sender)));
        ...
        returns (ValidationData vd)
    {
        ...
        vd = _validateActionPolicies(signerId, actionId, msg.sender, target, value, data, userOp);
    }
        ...
        returns (bytes4 sigValidationResult)
    {
        ...
        for (uint256 i; i < policies.length(smartAccount); i++) {
            ...
            if (
                !I1271Policy(policies.get(smartAccount, i)).check1271SignedAction(
                    id, msg.sender, sender, hash, signature
                )
            ) {
                return EIP1271_FAILED;
            }
    ...
    function _enablePermission(bytes calldata permissionData) internal returns (SignerId, address) {
        SignerId signerId = SignerId.wrap(bytes32(permissionData[0:32]).mixinNonce(getNonce(msg.sender)));
    ...
    function resetModule() public {
        ...
        // DO NOT NEED to clean userOpPolicies, actionPolicies, erc1271Policies as old signerIds are not reachable after
        // nonce increment
    }
    ...
    function renounceUserOpPolicy(bytes32 _signerId, address policy) external {
        ...
        _callSubModuleAndHandleReturnData(
            policy,
            abi.encodePacked(
                abi.encodeWithSelector(IERC7579Module.onUninstall.selector, SignerId.unwrap(signerId)),
                address(this), //append self address
    ...
    function renounceActionPolicy(bytes32 _signerId, ActionId actionId, address policy) external {
        ...
        _callSubModuleAndHandleReturnData(
            policy,
            abi.encodePacked(
                abi.encodeWithSelector(IERC7579Module.onUninstall.selector, id),
                address(this), //append self address
                msg.sender //append smart account address as original msg.sender
            )
        );
    ...
    function getSignerValidator(
        SignerId signerId,
        address smartAccount
    )
        public
        view
        returns (address signerValidator)
    {
        assembly {
